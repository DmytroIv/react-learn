...
import { Loader, RepoList } from '@/components/GithubFinderApp';
import { GithubContext } from '@/context/github/github.context';
import { FaCodepen, FaStore, FaUserFriends, FaUsers } from 'react-icons/fa';

const UserDetailsPage = () => {
  const { user, error, repos, getUserDetails } = useContext(GithubContext);
    ...
    return (
      <>
        <div className="w-full mx-auto lg:w-10/12">
            ...
            <div className="custom-card-image mb-6 md:mb-0">
              <div className="shadow-xl card image-full">
                ...
                <div className="card-body justify-end">
                  ...
                  <p className="flex-grow-0">{userLogin}</p>
                </div>
              </div>
            </div>
          ...
          {!!repos?.length && <RepoList repos={repos} />}
        </div>
      </>
    );
  }

  if (error) return <Navigate to="/github-finder" />;

  return <Loader />;
};

export default withLayout(UserDetailsPage);
